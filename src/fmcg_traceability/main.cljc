(ns fmcg_traceability.main
  "Kotodama WASM entrypoint for the Fmcg_traceability clean-room actor (L4) — Clojure port.
  ...
  facts (`fmcg_traceability.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "fmcg_traceability")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"fmcg_traceability.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "fmcg_traceability-compat" :tier tier :entities entities} 200])
...
(defn start! [] :fmcg_traceability-compat/ready)
