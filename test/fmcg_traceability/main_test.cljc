(ns fmcg_traceability.main-test
  "Contract + behavioral test for the fmcg_traceability-compat L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "fmcg_traceability-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'fmcg_traceability.main-test)]
